...
class Profile extends Component {
    renderSocialLinks(links) {
        ...
        return <div class="level is-mobile profile-social-links">
            {links.filter(link => typeof link === 'object').map(link => {
                return <a class="level-item button is-transparent is-marginless profile-social-link"
        ...
        return <div class="card widget" data-type="profile">
            <div class="card-content">
                <nav class="level">
                    <div class="level-item has-text-centered flex-shrink-1">
                        <div>
                            ...
                            {author ? <p class="title is-size-4 is-block profile-author-title" style={{'line-height': 'inherit'}}>{author}</p> : null}
                            {authorTitle ? <p class="is-block profile-author-subtitle">{authorTitle}</p> : null}
                ...
                {followLink ? <div class="level">
                    <a class="level-item button is-primary is-rounded" href={followLink} target="_blank" rel="me noopener"><i class="fab fa-github" aria-hidden="true"></i>&nbsp;&nbsp;{followTitle}</a>
                    {sponsorLink ? <a class="level-item button is-primary is-rounded" href={sponsorLink} target="_blank" rel="noopener"><i class="fas fa-heart" aria-hidden="true"></i>&nbsp;&nbsp;Sponsor</a> : null}
                </div> : null}
...
Profile.Cacheable = cacheComponent(Profile, 'widget.profile', props => {
    ...
    if ('sponsor_link' in widget && (typeof widget.sponsor_link !== 'string' || !widget.sponsor_link.startsWith('https://'))) {
        throw new Error('profile.sponsor_link must be a real HTTPS account URL');
    }
    ...
    return {
        ...
        sponsorLink: widget.sponsor_link,
