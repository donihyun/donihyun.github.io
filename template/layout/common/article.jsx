        ...
        return <Fragment>
            ...
            <div class="card">
                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`} role="article">
                    {/* Title */}
                    {page.title !== '' && index ? <h1 class="title is-3"><a class="link-muted" href={url_for(page.link || page.path)}>{page.title}</a></h1> : null}
                    {page.title !== '' && !index ? <h1 class="title is-3">{page.title}</h1> : null}
                    {/* Metadata */}
                    {page.layout !== 'page' ? <div class="article-meta is-size-7 is-uppercase level is-mobile">
                        <div class="level-left">
                            {/* Dates remain absolute, matching the reference template. */}
                            ...
                            {page.categories && page.categories.length ? <span class="level-item">
                                <i class="far fa-folder-open has-text-grey" aria-hidden="true"></i>&nbsp;
                                {(() => {
                                    const categories = [];
                            ...
                            {!index && plugins && plugins.busuanzi === true ? <span class="level-item" id="busuanzi_container_page_pv" dangerouslySetInnerHTML={{
                                __html: _p('plugin.visit_count', '<i class="far fa-eye" aria-hidden="true"></i>&nbsp;<span id="busuanzi_value_page_pv">Loading…</span>')
                            }}></span> : null}
                        </div>
                    </div> : null}
                    {/* Content/Excerpt */}
                    ...
                    {/* Tags */}
                    {page.tags && page.tags.length ? <Fragment>
                        <hr class="article-divider" />
                        <div class="article-tags is-size-7 is-uppercase">
                            <i class="fas fa-tags has-text-grey" aria-hidden="true"></i>&nbsp;
                            {page.tags.map((tag, i) => <span>
                                <a class="link-muted" rel="tag" href={url_for(tag.path)}>{tag.name}</a>
